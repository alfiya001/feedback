													...
													<form action="" method="post" onSubmit="return confirm('Are You Sure With Your Feedback')">
														<table>
															<tbody>
															 <?php 
															    $i = 1;
																$feed = mysql_fetch_assoc(mysql_query("SELECT * FROM ".$_SESSION['semtable']." WHERE staffid='$id' AND status='1'"));
															 	$result = mysql_query("SELECT * FROM feedbackarea");
																while($row = mysql_fetch_array($result)){  
															 ?>
														     <tr>
															 	<td width="200" height="30"><?php echo $row['name'];?></td><td width="200"><input type="range"  id="a<?php echo $i;?>" name="b[]" value="<?php echo $feed['f'.$i];?>" min='0' max='10'></td><td width="100" align="center" style="font-size:16px"><span id="displayval<?php echo $i;?>"><?php echo $feed['f'.$i];?></span></td>
															 </tr>
															 <?php 
															 		$i++;	
																}
